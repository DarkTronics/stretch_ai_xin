...
def main(
    ...
    id_to_name_hash = dict(semantic_sensor.seg_id_to_name)
    ...
    scene_graph = SceneGraph(parameters=vlm_parameters, instances=instances, hashmap=id_to_name_hash)
    scene_graph.update(instances)
    # relationships = scene_graph.get_relationships(debug=True)
    relationships_json = scene_graph.get_relationships_json()
    print(json.dumps(relationships_json, indent=2))
